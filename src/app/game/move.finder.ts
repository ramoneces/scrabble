...
export class MoveFinder {
  ...
  findMove(game: Game, player: Player): Move | undefined {
    const moves = mapMany(
  ...
  private squareCanStartFirstWord(
    ...
    return !![square, ...square.nexts[d].slice(0, player.rack.length - 1)].find(
  ...
  private squareCanStartWord(
    ...
    return square.nexts[d].slice(0, player.rack.length).some((s) => !!s.tile);
  }
  ...
  private findMovesInDirection(
    ...
    if (previousWordTiles.length === 0) {
      ...
      if (
        (isFirstMove &&
          !this.squareCanStartFirstWord(player, currentSquare!, d)) ||
        (!isFirstMove && !this.squareCanStartWord(player, currentSquare!, d))
      ) {
        return [];
      }
    }
    ...
    for (const tile of tiles) {
      ...
      for (const letter of letters) {
        ...
        if (usingTileFromRack) {
          ...
          } = this.searchConnectedWord(game, tile, letter, currentSquare, d);
  ...
  searchConnectedWord(
    ...
    let lexiconIndex: WordIndex | undefined = game.lexicon.index;
  ...
  private buildMove(
    ...
    const moveWord = this.buildMoveWord(game, word, wordTiles, wordSquares);
    return {
      player,
      moveWord,
      connectedWords,
      ...
      isBingo:
        moveWord.squares.filter((s) => !s.tile).length === game.rules.rackSize,
    };
  }
  ...
  private buildMoveWord(
    ...
    return {
      ...
      tiles,
      squares,
      score: this.computeWordScore(game, squares, tiles),
    };
  }

  private computeWordScore(
    game: Game,
    wordSquares: Square[],
    wordTiles: Tile[]
  ): number {
    ...
    if (tilesPlayed === game.rules.rackSize) {
      wordScore += 50; // Bingo!
    }
